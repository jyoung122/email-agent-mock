...
const guides: Record<string, ScreenGuide> = {
  dashboard: {
    ...
    exists: ['Executive metrics for volume, backlog, QA, and release readiness', 'Comparisons across institutions and departments', 'Exception and knowledge-health summaries', 'Shortcuts into queues that need attention'],
  ...
  workbench: {
    ...
    businessCase: 'Reviewers can make a release decision from one evidence-rich workspace instead of reconstructing context across email, forms, policies, and separate tools.',
  ...
  releaseQueue: {
    screen: 'Release Queue',
    purpose: 'The controlled staging area for QA-selected and approved responses before simulated release.',
    exists: ['Release batches grouped by institution, department, and mailbox', 'Locked population, mandatory review, random QA, approved, held, and released counts', 'Controls for schedule, hold, resume, QA expansion, and release', 'Confirmation safeguards for consequential actions'],
    ...
    painSolved: 'Avoids immediate unsupervised sends, opaque QA selection, and accidental release of held or policy-affected correspondence.',
    additions: ['Calendar and blackout-window management', 'Pre-release exception reports', 'Two-person approval for sensitive batches'],
    firstAction: 'Open the Registrar batch and compare approved, sampled, and held populations before release.',
  },
  controls: {
    screen: 'Agent Controls',
    purpose: 'The governance surface for configuring automation, QA, confidence, release, and emergency operating modes.',
    exists: ['Operating modes from normal automation through full review and pause', 'QA percentages and confidence thresholds', 'Release timing and business-hours safeguards', 'Visible knowledge-change and emergency controls'],
    ...
    firstAction: 'Review the current operating mode and the Registrar QA policy before changing release behavior.',
  },
  knowledge: {
    screen: 'Knowledge',
  ...
  administration: {
    ...
    exists: ['Institution, department, mailbox, user, and role records', 'Configured form definitions used as attachment-classification ground truth', 'Canonical extraction fields and validation rules', 'Routing, escalation, release, QA, tone, and retention settings'],
