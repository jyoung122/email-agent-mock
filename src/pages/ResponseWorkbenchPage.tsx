  ...
  return <div className="workbench-page">
    ...
    <div className="workbench-grid">
      <section className="workbench-panel source-panel" aria-label="Original message">
        <div className="workbench-panel-head"><div><span className="section-icon"><MessageSquareText size={17} /></span><div><h2>Original request</h2><p>{email.receivedAt}</p></div></div><Button variant="quiet" className="ssi-icon-button" aria-label="More message actions"><MoreHorizontal size={17} /></Button></div>
      ...
      <aside className="workbench-panel evidence-panel" aria-label="Evidence and controls">
        ...
        {email.assessment.warnings.length > 0 && <section className="evidence-section"><div className="evidence-heading warning"><AlertTriangle size={17} /><h2>Missing information</h2></div>{email.assessment.warnings.map((warning) => <div className="warning-card" key={warning}><strong>{warning}</strong><span>Release is blocked until resolved.</span></div>)}</section>}
