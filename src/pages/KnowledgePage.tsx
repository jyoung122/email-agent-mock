...
const impactOptions: Array<{ id: Impact; title: string; detail: string }> = [
  ...
  { id: 'hold-batch', title: 'Hold delivery batch', detail: 'Pause the affected Registrar batch until a reviewer decides.' },
  { id: 'continue', title: 'Continue with current settings', detail: 'Record the knowledge change while retaining the current delivery configuration.' },
]
...
export default function KnowledgePage() {
  ...
  return <div className="ops-page">
    ...
    {transcript?.changed && <section className="ops-banner warning"><ShieldAlert size={18} /><div><strong>Transcript policy change is active</strong><p>{transcript.affectedDraftIds.length} staged draft{transcript.affectedDraftIds.length === 1 ? '' : 's'} may be affected. The selected impact treatment is reflected in Agent Controls and Delivery Queue.</p></div></section>}
