    ...
    <section className="panel queue-panel" data-tour="work-queue-primary">
      ...
      <div className="data-table-wrap queue-table-wrap">
        <table className="data-table queue-table">
          <thead><tr><th>Sender / subject</th><th>Institution</th><th>Department</th><th>Mailbox</th><th>Request type</th><th>Status</th><th>Risk</th><th>QA status</th><th>Scheduled release</th><th>Assigned user</th><th>Received</th></tr></thead>
          <tbody>{filtered.map((email) => <tr key={email.id} className={email.id === PRIMARY_TRANSCRIPT_EMAIL_ID ? 'primary-scenario-row' : ''} tabIndex={0} onClick={() => openEmail(email)} onKeyDown={(event) => { if (event.key === 'Enter') openEmail(email) }} aria-label={`Open ${email.subject} from ${email.sender}`}><td className="queue-identity"><strong>{email.sender}</strong><span>{email.subject}</span>{email.assessment.warnings.length > 0 && <small>{email.assessment.warnings[0]}</small>}</td><td>{names.institution.get(email.institutionId)}</td><td>{names.department.get(email.departmentId)}</td><td>{names.mailbox.get(email.mailboxId)}</td><td>{email.requestType}</td><td><Badge>{email.status}</Badge></td><td><Badge tone={riskTone(email.risk)}>{email.risk} risk</Badge></td><td><span className="qa-cell">{email.qaStatus}</span></td><td className="numeric date-cell">{email.scheduledReleaseAt ?? '—'}</td><td>{email.assignedUserId ? names.user.get(email.assignedUserId) : <span className="muted">Unassigned</span>}</td><td className="numeric">{email.receivedAt.replace('Aug 2, 2026 · ', '')}</td></tr>)}</tbody>
        </table>
      </div>
