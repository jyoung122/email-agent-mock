...
export default function ReportingPage() {
  ...
  return <div className="leadership-page">
    ...
    <section className="report-summary" aria-label={`Executive summary for ${scope}`}>
      {[['Estimated staff hours saved', '481', 'hours'], ['Response-time reduction', '42%', 'vs. baseline'], ['Automated delivery rate', '64%', '1,201 of 1,876'], ['QA pass rate', '96%', '702 of 731'], ['Forms normalized', '132', '18 missing forms caught']].map(([label, value, sub]) => <article key={label}><span>{label}</span><strong>{value}</strong><small>{sub}</small></article>)}
    </section>
    <div className="report-callout"><Info size={17} /><span><b>How to read this period.</b> Automated delivery excludes messages selected for mandatory review; QA pass rate reflects completed human decisions, not automated confidence.</span></div>
